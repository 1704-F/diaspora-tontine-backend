...
class IncomeEntryController {
  constructor() {
    ...
    this.createIncomeEntry = this.createIncomeEntry.bind(this);
    this.getIncomeEntries = this.getIncomeEntries.bind(this);
    this.getIncomeEntryDetails = this.getIncomeEntryDetails.bind(this);
    this.validateIncomeEntry = this.validateIncomeEntry.bind(this);
    this.rejectIncomeEntry = this.rejectIncomeEntry.bind(this);
    this.generateReceipt = this.generateReceipt.bind(this);
    this.getIncomeStatistics = this.getIncomeStatistics.bind(this);
    this.updateIncomeEntry = this.updateIncomeEntry.bind(this);
    this.cancelIncomeEntry = this.cancelIncomeEntry.bind(this);
    this.resubmitIncomeEntry = this.resubmitIncomeEntry.bind(this);
    this.uploadDocument = this.uploadDocument.bind(this);
    this.exportIncomeData = this.exportIncomeData.bind(this);
    this.getPendingValidations = this.getPendingValidations.bind(this);
  ...
  async getIncomeEntries(req, res) {
    ...
    try {
      ...
      const { count, rows: incomeEntries } = await IncomeEntry.findAndCountAll({
        where: whereClause,
        include: [
          {
            model: Association,
    ...
    try {
      ...
      const byStatus = await IncomeEntry.findAll({
        ...
        attributes: [
          "status",
          [
            IncomeEntry.sequelize.fn("COUNT", IncomeEntry.sequelize.col("id")),
            "count",
          ],
          [
            IncomeEntry.sequelize.fn(
              "SUM",
              IncomeEntry.sequelize.col("amount")
            ),
            "total",
          ],
        ],
    ...
    try {
      ...
      if (amount !== undefined) {
        ...
        updateData.netAmount =
          parseFloat(amount) - parseFloat(fees || incomeEntry.fees || 0);
      }
      if (grossAmount !== undefined)
        updateData.grossAmount = parseFloat(grossAmount);
      if (fees !== undefined) {
        updateData.fees = parseFloat(fees);
        updateData.netAmount =
          parseFloat(amount || incomeEntry.amount) - parseFloat(fees);
      }
      if (currency !== undefined) updateData.currency = currency;
      if (sourceType !== undefined) updateData.sourceType = sourceType;
      if (sourceName !== undefined)
        updateData.sourceName = isAnonymous ? null : sourceName;
      if (sourceDetails !== undefined)
        updateData.sourceDetails = isAnonymous ? null : sourceDetails;
      ...
      if (manualReference !== undefined)
        updateData.manualReference = manualReference;
      ...
      if (usageRestrictions !== undefined)
        updateData.usageRestrictions = usageRestrictions;
      if (publiclyVisible !== undefined)
        updateData.publiclyVisible = publiclyVisible;
      if (thanksRequired !== undefined)
        updateData.thanksRequired = thanksRequired;
  ...
  async cancelIncomeEntry(req, res) {
    try {
      ...
      await incomeEntry.update({
        status: "cancelled",
        internalNotes: `Annulée par ${req.user.firstName} ${
          req.user.lastName
  ...
  async exportIncomeData(req, res) {
    try {
      ...
      const exportData = incomeEntries.map((entry) => ({
        ...
        "Validé par": entry.validatedByUser
          ? `${entry.validatedByUser.firstName} ${entry.validatedByUser.lastName}`
          : "",
        "Date validation": entry.validatedAt
          ? entry.validatedAt.toISOString().split("T")[0]
          : "",
        ...(includeDetails && {
  ...
  async getPendingValidations(req, res) {
    try {
      ...
      const stats = {
        ...
        totalAmount: pendingEntries.reduce(
      ...
      pendingEntries.forEach((entry) => {
        stats.byType[entry.incomeType] =
          (stats.byType[entry.incomeType] || 0) + 1;

        // Vérifier urgence (plus de 7 jours)
        const daysSinceCreation = Math.floor(
          (new Date() - entry.createdAt) / (1000 * 60 * 60 * 24)
        );
